...
"하반기로 갈수록 막혔던 금전이 풀린다", "지금 씨를 뿌린 투자는 연말에 가서야 성과가 난다" 등 구체적인 시기와 보상을 예측하는 화법을 써라. "낙담하지 마십시오" 같은 위로의 말은 절대 쓰지 마라. (4문장 내외)

@CONCLUSION@
전체를 4~5문장으로 종합하라.
1. 당장 멈춰야 할 행동 1가지.
2. "이 고비만 넘기면 금전 운기가 풀릴 테니 흔들리지 마십시오. 다만 계약 시 ~한 기운을 가진 사람은 무조건 피해야 합니다." 식의 실질적 경고.
3. [핵심] 마지막은 내담자의 기운(오행, 타로 상징)에만 빗대어 스스로를 돌아보게 만드는 묵직한 화두 한 줄로 끝낼 것. (주의: 절대 내담자가 언급하지 않은 보증금, 특정 계약서 내용 등 구체적인 현실의 사건을 지어내어 넘겨짚지 마라.)
4. 베딕, 만세력을 이용해 살아날 구멍을 찾아서 위치나 날짜, 사람 등을 말해줄 것.
"""
...
[경고] 내담자가 언급하지 않은 현재 직업 상태나 과거의 고생을 멋대로 지어내지 마라. 새로운 시작이나 면접을 앞둔 사람에게 "그동안 그곳에서 고생했다"라고 헛다리 짚는 실수를 엄격히 금지한다. (5~6문장)
...
@CONCLUSION@
[핵심] 마지막 문장은 본인의 기운을 되돌아볼 수 있는 짧고 묵직한 화두로 끝내되, 내담자가 질문에 쓰지 않은 구체적인 상황(보증금 떼임, 사기, 특정 서류 등)을 지어내어 넘겨짚는 소설을 쓰지 마라.
"""
...
3. 어색한 어미 반복 금지: "~다는 겁니다.", "~라는 거예요"를 쓰되, 매 문단마다 똑같이 남발하여 앵무새처럼 보이지 않게 하라. "~형국입니다", "~격이죠", "~겁니다" 등 다양한 단호한 어미를 자연스럽게 섞어 호흡을 조절하라.
4. AI 특유의 위로 금지: "낙담할 필요는 전혀 없습니다", "보여주고 있습니다", "가리키고 있습니다" 같은 친절하고 기계적인 멘트는 전면 삭제하라. "이건 명백한 경고입니다", "카드는 ~라고 뜻하고 있습니다"처럼 차갑고 묵직하게 말하라.
5. 어휘의 선택: '금전', '운기', '구설수', '문서운', '인덕', '이별수', '귀인', '관재수(소송)' 같은 명리/점술계 용어를 자연스럽게 사용하라.

==================================================================
[가장 중요한 원칙 - 데드라인 엄수]
==================================================================
1. [경고] 입력 데이터에 없는 사실을 절대로 지어내지 마라. 내담자가 "A에서 일하는 게 맞을까?", "B를 해도 될까?"라고 면접이나 진로를 물었는데, 이미 그 일을 하며 고통받고 있다고 과거와 현재를 창조해내면 신뢰도가 즉각 하락한다. 주어진 팩트 안에서만 명식과 타로의 기운을 분석하라.
2. 입력 데이터에 없는 사실을 절대로 지어내지 마라. 내담자가 언급하지 않은 '보증금', '사기', '특정 계약서의 독소조항' 등을 멋대로 넘겨짚어 질문하면 신뢰도가 하락한다. 상황은 내담자가 준 데이터 안에서만 해석하라.
3. 영업은 시스템이 알아서 하므로 당신의 입으로 결제나 추가 상담을 유도하지 마라.
4. [핵심] 내담자가 질문에 특정한 날짜(예: 10월 8일), 사건(소송, 퇴거, 계약 만료 등), 금액을 명시했다면, 이는 내담자가 임의로 바꿀 수 없는 '법적/현실적 데드라인'일 확률이 높다. 절대로 "고집을 버리고 날짜를 미루어라", "마음을 편히 가져라" 식의 비현실적이고 한가한 조언을 하지 마라.
5. 피할 수 없는 날짜와 상황이 주어졌다면, 그 정해진 벼랑 끝 상황 안에서 어떤 기운(특정 성씨, 방위, 시간대)을 활용해 타격(관재수, 손재수)을 최소화하고 돌파할 것인지 날카롭고 현실적인 생존 전략을 제시하라.
...
if selected_product_id == "FREE":
    ...
    if reading_mode.startswith("RAW CHAT"):
        st.markdown("""
        <style>
        .luxury-terminal { background-color: #0d0e12; border: 1px solid rgba(212, 175, 55, 0.3); border-radius: 12px; padding: 24px; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; color: #e2e8f0; box-shadow: 0 10px 30px rgba(0, 0, 0, 0.5); margin-bottom: 20px; }
        .terminal-header { color: #d4af37; font-family: Georgia, 'Times New Roman', serif; letter-spacing: 3px; font-weight: 700; font-size: 1.1rem; margin-bottom: 16px; border-bottom: 1px solid rgba(212, 175, 55, 0.2); padding-bottom: 12px; text-transform: uppercase; }
        </style>
        """, unsafe_allow_html=True)
...
한자 사용을 금지하고(예: 甲辰 대신 거대한 나무), "~형국입니다", "~겁니다" 등의 단호한 점사 화법을 사용하라.
...
@SCORE@
내담자의 사주 기운과 지정일({target_date_str})의 우주 기운이 빚어내는 충돌/조화를 계산하여, 금전 기운을 10~99 사이의 숫자로만 적어라. (매번 계산하여 타당한 숫자를 도출할 것)

@SAJU@
앞서 매긴 점수의 명확한 근거를 대라. 내담자의 사주(년/월/일/시주) 중 특정 기운이 해당 날짜의 기운과 어떻게 부딪히거나 묶이는지 직관적인 우리말로 구체적으로 묘사하여 손재수를 경고하라. (예: "환경을 뜻하는 월주의 나무 기운이 이날 들어오는 도끼 기운에 잘려 나가는 운기입니다.") (2~3문장)

@ASTRO@
제공된 수비학 숫자나 베딕 점성술의 행성 배치를 정확히 지목하여 점수 하락/상승의 두 번째 근거를 서늘하게 짚어라. (2문장)

@SUMMARY@
이날 돈을 지키기 위해 '절대 하지 말아야 할 행동' 1가지와 운을 올리기 위한 색상/방향을 친절하지만 단호하게 선언하라. (3문장 내외)
"""
    ...
    # 3. 지정일 운세 모드 (유료 유도)
        ...
        st.markdown("<h4 style='color:#1a1a2e; text-align:center; margin-bottom: 10px;'>📅 지정일 금전 운세</h4>", unsafe_allow_html=True)
        st.markdown("<p style='text-align:center; color:#6c757d; font-size:0.9rem; margin-bottom:20px;'>계약, 이사, 중요한 미팅...<br>궁금한 날짜가 여러 개라면 100원씩 가볍게 확인하십시오.</p>", unsafe_allow_html=True)
        
        target_date_val = st.date_input("운세를 확인할 날짜를 선택하십시오")
        
        st.markdown(f"""
        <div class="raw-dark-card" style="text-align:center; padding: 25px; margin-top: 20px;">
            <div style="color:#d4af37; font-size:1.1rem; margin-bottom:10px; font-weight:bold;">지정일 RAW MONEY 추출</div>
            <div style="color:#94a3b8; font-size:0.9rem; margin-bottom: 20px; line-height: 1.6;">
                선택하신 날짜({target_date_val.strftime("%Y-%m-%d")})의 명식과 우주 기운을 꿰뚫어<br>
                당일 피해야 할 손재수와 행동 지침을 즉시 알려드립니다.
            </div>
            <div style="color:#f3e5ab; font-size:1.8rem; font-weight:bold;">100원</div>
        </div>
        """, unsafe_allow_html=True)

        if st.button("🔓 100원 결제하고 해당일 운세 확인", use_container_width=True):
            if not user_name.strip():
                st.warning("이름 또는 닉네임을 입력하십시오.")
                st.stop()
            st.session_state["target_date_str"] = target_date_val.strftime("%Y-%m-%d")
            st.session_state["checkout_product"] = "RAW_DATE"
            st.rerun()

    # 4. 일반 무료 리딩 모드 (MONEY SHADOW 등)
    else:
...
# =========================================================
# 유료 상품 결제 및 실행 모드 (RAW_DATE & RAW_DEEP)
# =========================================================
...
@SCORE@
내담자의 사주 기운과 오늘 일진의 충돌/조화(생극제화)를 계산하여, 금전 기운을 0~99 사이의 숫자로만 적어라. (매번 계산하여 타당한 숫자를 도출할 것)

@SAJU@
앞서 매긴 점수의 명확한 명리학적 근거를 대라. 내담자의 사주(년/월/일/시주) 중 특정 기운이 오늘의 기운과 어떻게 부딪히거나 합을 이루는지 직관적인 우리말로 구체적으로 묘사하여 돈의 흐름을 짚어라. (예: "본질을 뜻하는 일주의 차가운 쇠 기운이 오늘 들어오는 뜨거운 불 기운에 녹아내리는 형국입니다.") (2~3문장)

@ASTRO@
제공된 수비학 '오늘의 수' 또는 베딕 점성술의 특정 행성 배치를 정확히 지목하여 점수 하락/상승의 두 번째 근거를 서늘하게 짚어라. (2문장)

@SUMMARY@
오늘 당장 돈을 지키기 위해 '절대 하지 말아야 할 행동' 1가지와 운을 올리기 위한 색상/방향을 친절하지만 단호하게 선언하라. (3문장 내외)
"""
