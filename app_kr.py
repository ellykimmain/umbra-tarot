    ...
    if reading_mode.startswith("RAW CHAT"):
        ...
        if not st.session_state["chat_initialized"]:
            ...
            if user_input := st.chat_input("오라클에게 추가로 따져묻거나 질문하세요..."):
                ...
                [필수 지침: 결말 유도 및 반전 제시]
                1. 길고 장황한 설명이나 위로는 절대 금지한다. 기존 분량의 '딱 절반' 수준으로 짧고 압축적이고 서늘하게 작성하라.
                2. 내담자의 태도를 사주 명식(오행, 십신)과 베딕 점성술(행성, 하우스)의 관점에서 날카롭게 해부하되, 절망만 주지 말고 **"잠깐, 명식의 다른 구도를 보니 탈출구(해결점)의 단서가 숨어 있다"**는 뉘앙스를 풍겨라.
                3. 마지막은 벼랑 끝에서 유저를 구하는 질문이 아니라, 다음과 같은 맥락의 선택지형 질문으로 끝낼 것.
                   (예시: "하지만 명식의 특정 구간을 다시 교차 검증해보면, 베딕의 특정 하우스나 사주의 미세한 합(合) 속에 이 위기를 돌파할 의외의 반전 코드가 숨어 있다. 이 실마리를 더 파헤쳐 볼 것인가?")
