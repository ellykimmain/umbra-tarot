    ...
    if reading_mode.startswith("RAW CHAT"):
        ...
        if not st.session_state["chat_initialized"]:
            if st.button(">> CONNECT ORACLE (상담 채널 열기)"):
                ...
                1. 일반적인 심리 상담, 위로, 뻔한 위기 대처 가이드(예: "변호사를 선임해라", "시간을 벌어라" 같은 대책)를 절대 말하지 마라.
                2. 모든 진단과 해석은 반드시 위 [점술 데이터] 안의 명식(사주 글자, 십신, 베딕 행성, 수비학 숫자)에서 그 원인을 찾아내어 연결해야 한다. (예: "왜 그 일이 일어났는가?"에 대해 사주 원국의 어떤 합충(合沖)이나 행성 배치 때문인지 물리적/운명적으로 해석할 것)
                3. 분량은 기존의 절반 수준으로 짧고 압축적으로 꺾어 누르듯 작성하라.
