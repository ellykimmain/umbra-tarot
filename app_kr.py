...
if selected_product_id == "FREE":
    ...
    # 2. 오늘의 운세 모드
        ...
        if st.button("오늘의 RAW MONEY 확인하기", use_container_width=True):
            ...
            # 💡 [핵심 해결] 파이썬이 직접 0~99 사이의 역동적인 점수를 생성합니다.
            import random
            daily_score = random.randint(0, 99)
...
@SAJU@
오늘의 금전 점수가 {daily_score}점인 이유를 설명하라. 점수가 50점 이상이면 재물이 들어오는 길운으로, 미만이면 손실을 주의해야 하는 흉운으로 해석할 것. 내담자의 사주(년/월/일/시주) 중 특정 기운이 오늘 일진과 어떻게 부딪히거나 합을 이루는지 직관적인 우리말로 구체적으로 묘사하라. (2~3문장)

@ASTRO@
제공된 수비학 숫자나 베딕 점성술의 특정 행성 배치를 지목하여 점수({daily_score}점)에 대한 두 번째 근거를 서늘하게 짚어라. (2문장)

@SUMMARY@
오늘 당장 돈을 지키거나 불리기 위해 '절대 하지 말아야 할 행동' 1가지와 운을 올리기 위한 색상/방향을 친절하지만 단호하게 선언하라. (3문장 내외)
"""
    ...
    if selected_product_id == "RAW_DATE":
        ...
        if st.session_state.get("raw_date_completed"):
            ...
            if st.button("결제하고 지정일 운세 확인하기 (테스트)", use_container_width=True):
                ...
                # 💡 [핵심 해결] 파이썬이 직접 0~99 사이의 역동적인 점수를 생성합니다.
                import random
                target_score = random.randint(0, 99)
...
@SAJU@
지정일의 금전 점수가 {target_score}점인 이유를 설명하라. 점수가 50점 이상이면 재물이 들어오는 길운으로, 미만이면 손실을 주의해야 하는 흉운으로 해석할 것. 내담자의 사주 기운이 지정일의 기운과 어떻게 부딪히거나 묶이는지 직관적인 우리말로 묘사하라. (2~3문장)

@ASTRO@
제공된 수비학 숫자나 베딕 점성술의 행성 배치를 지목하여 점수({target_score}점)에 대한 두 번째 근거를 서늘하게 짚어라. (2문장)

@SUMMARY@
이날 돈을 지키거나 불리기 위해 '절대 하지 말아야 할 행동' 1가지와 운을 올리기 위한 색상/방향을 친절하지만 단호하게 선언하라. (3문장 내외)
"""
